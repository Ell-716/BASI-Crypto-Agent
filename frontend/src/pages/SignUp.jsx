  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-no-repeat bg-cover bg-center" style={{ backgroundImage: "url('/background.jpg')", backgroundPosition: 'center 50%' }}>
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-start items-start pt-40 pl-20 text-white">
        ...
        <p className="text-2xl font-medium mt-8">Get AI-powered buy, sell or hold predictions!</p>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-8 lg:p-16">
        <div className="w-full max-w-md bg-white bg-opacity-90 border-2 border-blue-600 rounded-3xl p-6 sm:p-10 shadow-xl">
          <h2 className="text-2xl sm:text-3xl font-bold text-center text-blue-600 mb-6">Sign Up</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <input name="user_name" value={form.user_name} onChange={handleChange} type="text" placeholder="Username" required className="w-full px-5 py-4 text-lg border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600" />
              {fieldErrors.user_name && <p className="text-red-600 text-sm mt-1">{fieldErrors.user_name}</p>}
            </div>

            <div>
              <input name="email" value={form.email} onChange={handleChange} type="email" placeholder="Email" required className="w-full px-5 py-4 text-lg border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600" />
              {fieldErrors.email && <p className="text-red-600 text-sm mt-1">{fieldErrors.email}</p>}
            </div>

            <div>
              <div className="relative">
                <input name="password" value={form.password} onChange={handleChange} type={showPassword ? 'text' : 'password'} placeholder="Password" required className="w-full px-5 py-4 text-lg border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 pr-12" />
                <div className="absolute inset-y-0 right-4 flex items-center cursor-pointer text-gray-500 hover:text-gray-700" onClick={() => setShowPassword(!showPassword)}>
            ...
            <div>
              <input name="confirmPassword" value={form.confirmPassword} onChange={handleChange} type="password" placeholder="Confirm Password" required className="w-full px-5 py-4 text-lg border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600" />
              {fieldErrors.confirmPassword && <p className="text-red-600 text-sm mt-1">{fieldErrors.confirmPassword}</p>}
            </div>

            <button type="submit" className="w-full bg-blue-600 text-white text-xl font-semibold py-4 rounded-xl hover:bg-blue-700 transition mt-4">
              Create Account
            </button>
          </form>
